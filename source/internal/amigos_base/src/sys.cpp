...
#endif
int Sys::GetIniInt(const std::string &section, std::string key, int intDefault)
{
    std::string strTmp;
...
unsigned int Sys::GetIniUnsignedInt(const std::string &section, std::string key, unsigned int uintDefault)
{
    std::string strTmp;
...
char* Sys::GetIniString(const std::string &section, std::string key, char *pDefaultStr)
{
    std::string strTmp;
...
void Sys::Implement(std::string &strKey)
{
    ...
    //[FILE_CH0_DEV0]    #This is section key(strKey) must not be repeatable
    //MOD=FILE           #This is module name(strModName), allow repeation, and it is for class instance search.
    //CHN=0
    //DEV=0
    if (!FindBlockId(strKey, strModName))
        ASSERT(0);
    if (!Sys::FindBlock(strKey))
    {
        auto itInitMap = AmigosModuleInit::initMap.find(strModName);
        if (itInitMap == AmigosModuleInit::initMap.end())
            ASSERT(0);
        itInitMap->second(strKey);
...
void Sys::InitSys(std::string strIniPath)
{
    std::map<std::string, Sys *> maskMap;

    CreateObj(strIniPath);
    maskMap.clear();
    Begin(maskMap);
}
void Sys::DeinitSys()
...
void Sys::CreateObj(std::string strIniPath)
{
    unsigned int i = 0;
    Sys *pClass = NULL;
...
void Sys::SetCurInfo(const std::string &strKey, unsigned int modId)
{
            ...
            case E_STREAM_AUDIO_CODEC_DATA:
            {
                ...
                case E_STREAM_PCM:
                {
                    ...
                    if (modId == E_SYS_MOD_AO)
                    {
                        ...
                        s32Ret = MI_AO_SendFrame((MI_AUDIO_DEV)pInstance->stModDesc.devId, (MI_AO_CHN)pInstance->stModDesc.chnId, &stAoFrame, 40);
                        if (MI_SUCCESS != s32Ret)
                        {
