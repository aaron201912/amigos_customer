...
void Inject::LoadDb()
{
    ...
    for (itMapInjectOut = mapModOutputInfo.begin(); itMapInjectOut != mapModOutputInfo.end(); itMapInjectOut++)
    {
        ...
        if (mapInjectOutInfo[itMapInjectOut->first].bEnableOsd)
        {
            ...
            mapInjectOutInfo[itMapInjectOut->first].uintOsdTargetPortId = GetIniUnsignedInt(itMapInjectOut->second.curIoKeyString, "OSD_ATTACH_PORT");
            mapInjectOutInfo[itMapInjectOut->first].uintOsdTargetPortWid = GetIniUnsignedInt(itMapInjectOut->second.curIoKeyString, "OSD_ATTACH_PORT_WIDTH");
            mapInjectOutInfo[itMapInjectOut->first].uintOsdTargetPortHei = GetIniUnsignedInt(itMapInjectOut->second.curIoKeyString, "OSD_ATTACH_PORT_HEIGHT");
        }
    }
}
...
#ifndef CONFIG_SIGMASTAR_CHIP_I2M
...
#endif
#endif
    unsigned char bUpdateOsd = FALSE;
    unsigned uintOsdHandleId = 0;

    if (pThisClass->mapInjectOutInfo[pReceiver->uintPort].bEnableOsd)
    {
        ...
        for (itVectNext = itMapInjectOut->second.vectNext.begin(); itVectNext != itMapInjectOut->second.vectNext.end(); itVectNext++)
        {
            ...
            if (s32Ret == MI_SUCCESS)
            {
                ...
                if (itMapInjectOutInfo->second.bEnableOsd)
                {
                    ...
                    stChnPort.s32OutputPortId = itMapInjectOutInfo->second.uintOsdTargetPortId;
                    stChnAttr.bShow = TRUE;
                    stChnAttr.stPoint.u32X = ((itMapInjectOutInfo->second.uintOsdTargetPortWid > itMapInjectOutInfo->second.uintOsdWidth)?
                                             (itMapInjectOutInfo->second.uintOsdTargetPortWid - itMapInjectOutInfo->second.uintOsdWidth): itMapInjectOutInfo->second.uintOsdTargetPortWid) / 2;
                    stChnAttr.stPoint.u32Y = ((itMapInjectOutInfo->second.uintOsdTargetPortHei > itMapInjectOutInfo->second.uintOsdHeight)?
                                             (itMapInjectOutInfo->second.uintOsdTargetPortHei - itMapInjectOutInfo->second.uintOsdHeight): itMapInjectOutInfo->second.uintOsdTargetPortHei) / 2;
#ifndef CONFIG_SIGMASTAR_CHIP_I2
...
AMIGOS_MODULE_INIT("INJECT", E_SYS_MOD_INJECT, Inject);
