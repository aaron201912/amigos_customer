        ...
        switch (pStreamData->stInfo.eStreamType)
        {
            ...
            case E_STREAM_VIDEO_CODEC_DATA:
            {
                switch (pStreamData->stInfo.stEsInfo.enVideoCodecFmt)
                {
                    ...
                    {
#if 0
                        unsigned char au8Header[16];
                        static int cnt = 0;

                        if (cnt == 1200)
                            break;
                        for (unsigned char i = 0; i < pStreamData->stEsData.uintPackCnt; i++)
                        {
...
int File::CreateSender(unsigned int outPortId)
{
    ...
    stTemAttr.bDropEvent = 0;
    TemOpen(mapModOutputInfo[outPortId].curIoKeyString.c_str(), stTemAttr);

    return 0;
}
int GetOneFrameYUV420(int srcFd, char *pYData, char *pUvData, int ySize, int uvSize)
{
...
int GetOneFrame(int srcFd, char *pYuvData, int intSize)
{
    off_t current = lseek(srcFd,0L, SEEK_CUR);
    off_t end = lseek(srcFd,0L, SEEK_END);
...
void * File::SenderMonitor(ST_TEM_BUFFER stBuf)
{
    ...
    unsigned int u32FrameLen = 0;
    unsigned char au8Header[32] = {0};
    int ret = -1;
    int y_size = 0;
    int uv_size = 0;
    int dataSize = 0;
        ...
        case E_STREAM_VIDEO_CODEC_DATA:
        {
            ...
            switch (stFileStreamData.stInfo.stEsInfo.enVideoCodecFmt)
            {
                ...
                case E_STREAM_JPEG:
                {
                    ...
                    u32FrameLen = U32VALUE(au8Header, 4);
                    pu8Buf = (unsigned char *)malloc(u32FrameLen);
                    ASSERT(pu8Buf);
...
AMIGOS_MODULE_INIT("FILE", E_SYS_MOD_FILE, File);
