...
#define AMIGOS_INFO(fmt, args...) do {printf("\033[1;32m[AMI_INFO][%s][%d]", __FUNCTION__, __LINE__);printf(fmt, ##args);printf("\033[0m");}while(0)
#define AMIGOS_ERR(fmt, args...) do {printf("\033[1;31m[AMI_ERR][%s][%d]", __FUNCTION__, __LINE__);printf(fmt, ##args);printf("\033[0m");}while(0)
...
class Sys
{
    public:
        ...
        static void InitSys(std::string strIniPath);
        static void DeinitSys();
        static void CreateObj(std::string strIniPath);
        ...
        {
            public:
                AmigosModuleInit(void (*preLoad)(void))
                {
                    preLoad();
                }
                virtual ~AmigosModuleInit() {}
                static std::map<std::string, void (*)(const std::string &)> initMap;
        };
        ...
        {
            public:
                explicit SysChild(const std::string &sectionKey, E_SYS_MOD enModId)
                {
                    Sys *pClass;

                    pClass = new (std::nothrow) SYSCHILD;
                    assert(pClass);
                    pClass->SetCurInfo(sectionKey, enModId);
                }
                virtual ~SysChild(){};
        };
    protected:
        ...
        //INI operation
        static int GetIniInt(const std::string &section, std::string key, int intDefault = -1);
        static unsigned int GetIniUnsignedInt(const std::string &section, std::string key, unsigned int uintDefault = -1);
        static char *GetIniString(const std::string &section, std::string key, char *pDefaultStr = NULL);
    ...
    private:
        void SetCurInfo(const std::string &strKey, unsigned int modId);
        void BuildModTree();
        ...
        static bool FindBlockId(std::string &strKey, std::string &strModName)
        {
            strModName = GetIniString(strKey, "MOD");
            return strModName.empty() ? false : true;
        }
...
#define AMIGOS_MODULE_INIT(__name, __modid, __class)                                      \
        static void __amigos_early_init_load_##__class(const std::string &key) {          \
            Sys::SysChild<__class> __class(key, __modid);                                 \
        }                                                                                 \
        void __amigos_early_init_preload_##__class() {                                    \
            AMIGOS_INFO("Preload Class %s, Name %s, Id %d\n", #__class, __name, __modid); \
            Sys::AmigosModuleInit::initMap[__name] = __amigos_early_init_load_##__class;  \
        }
#endif
